...
//         val conf = new SparkConf().setMaster("yarn").setAppName("My App") //tset yarn
//         val sc = new SparkContext(conf)
//         val point_hadoop_file_path = "/wedrive_data/raw_data/TB_TRACKING2_DATA_20201213.sql"
...
//         df.coalesce(1).sort($"key", $"id").write.format("com.databricks.spark.csv").option("header","true").save("/ysh/20200422/all")
...
//         correct_type2.coalesce(1).sort($"key", $"id").write.format("com.databricks.spark.csv").option("header","true").save("/ysh/20201213/type_1")
...
//         incorrect_type2.coalesce(1).sort($"key", $"id").write.format("com.databricks.spark.csv").option("header","true").save("/ysh/20200404/type_2")
